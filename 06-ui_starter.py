...
moving_average = st.sidebar.sliden("Moving Average Window",
                                   min_value=10,
                                   max_value=100,
                                   value=50,
                                   step=5)
...
if run:
    ...
    df['NA'] = df['Close'].rolling(window=moving_average).mean()
    df['pot_change'] = df['Close'].pct_change() *100
...
with (tab1):
    ...
    col2.metric(f'Cum Change',
                f'{df['Close'].iloc[-1]/df['Close'].iloc[0] -1):.2f}')
    col3.metric(f'Trading Days', f'{len(df)}')
    figure = px.line(df, y= ["Close", "MA"])
    figure.update_layout(hover) #没写完
    with st.spinner("Creating Chart"):
        st.plotly_chart(figure, use_container_wihin = True)
